...
layout(location = 1) in vec3 outPositionWS;
layout(location = 2) in vec3 outNormalWS;
...
void main() {
    if (UBO.userInput.x == 1) {
        vec3 scaledNormal = 0.5f * normalize(outNormalWS) + 0.5f;
    ...
    if (UBO.userInput.y == 1) {
        float F0 = 0.1f;
        vec3 V = normalize(UBO.camera_pos.xyz - outPositionWS);
        float cos_theta = max(dot(normalize(outNormalWS), V), 0.0f);
